...
/**
 * Dc_wpnews_Settings Class
 *
 * @class Dc_wpnews_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	Dc_wpnews
 * @author Jeffikus
 */
final class Dc_wpnews_Settings {
	/**
	...
	/**
	 * Main Dc_wpnews_Settings Instance
	 *
	 * Ensures only one instance of Dc_wpnews_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main Dc_wpnews_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool)apply_filters( 'dc-wpnews-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'dc-wpnews-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'dc-wpnews-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= Dc_wpnews()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'dc-wpnews-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'dc-wpnews-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'dc-wpnews' );
		$settings_sections['special-fields'] = __( 'Special Fields', 'dc-wpnews' );
		...
		return (array)apply_filters( 'dc-wpnews-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				...
				$settings_fields['text'] = array(
												'name' => __( 'Example Text Input', 'dc-wpnews' ),
												'type' => 'text',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'dc-wpnews' )
											);
				$settings_fields['textarea'] = array(
												'name' => __( 'Example Textarea', 'dc-wpnews' ),
												'type' => 'textarea',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'dc-wpnews' )
				...
				$settings_fields['checkbox'] = array(
												'name' => __( 'Example Checkbox', 'dc-wpnews' ),
												'type' => 'checkbox',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'dc-wpnews' )
				...
				$settings_fields['radio'] = array(
												'name' => __( 'Example Radio Buttons', 'dc-wpnews' ),
												'type' => 'radio',
												'default' => '',
												'section' => 'standard-fields',
												'options' => array(
																	'one' => __( 'One', 'dc-wpnews' ),
																	'two' => __( 'Two', 'dc-wpnews' ),
																	'three' => __( 'Three', 'dc-wpnews' )
															),
												'description' => __( 'Place the field description text here.', 'dc-wpnews' )
											);
				$settings_fields['select'] = array(
													'name' => __( 'Example Select', 'dc-wpnews' ),
													'type' => 'select',
													'default' => '',
													'section' => 'standard-fields',
													'options' => array(
			...
			case 'special-fields':
				...
				$settings_fields['select_taxonomy'] = array(
													'name' => __( 'Example Taxonomy Selector', 'dc-wpnews' ),
		...
		return (array)apply_filters( 'dc-wpnews-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'dc-wpnews-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'dc-wpnews-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'dc-wpnews-' . $v, array() );
